...
//	cprintf("envID = %d\n",envID);
	...
	cprintf("checking REPLACEMENT fault handling of STACK pages... \n");
	{
		...
		if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  10){
			panic("Unexpected extra/less pages have been added to page file");

		}

		if( (freePages - (sys_calculate_free_frames() + sys_calculate_modified_frames())) != 0 ){

	...
	for (int i = 0; i < numOfAllocs; ++i)
	{
		for (int j = 0; j < allocCntPerSize; ++j)
		{
			...
			if(va == NULL || (va != (curVA + sizeOfMetaData())))
			{
				if (is_correct)
				{
					struct BlockMetaData* block = va-sizeOfMetaData();

					is_correct = 0;
//					cprintf("end of block %x\n", endVAs[idx]);
//					cprintf("begin of next block %x\n", endVAs[idx]+1*sizeof(char));
//					cprintf("beginning of next %x\n",block->prev_next_info.le_next);
//					cprintf("curVA is: %x\n",curVA);
//					cprintf("va without metadata: %x\n", va-sizeOfMetaData());
	...
	for (int i = 0; i < numOfAllocs; ++i)
	{
		for (int j = 0; j < allocCntPerSize; ++j)
		{
			...
			if(va == NULL || (va != (curVA + sizeOfMetaData())))
				...
				cprintf("test_free_block #1.%d: WRONG ALLOC - alloc_block_FF return wrong address. Expected %x, Actual %x\n", idx, curVA + sizeOfMetaData() ,va);
			curVA += allocSizes[i] ;
			*(startVAs[idx]) = idx ;
			*(midVAs[idx]) = idx ;
			*(endVAs[idx]) = idx ;

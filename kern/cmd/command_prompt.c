...
int process_command(int number_of_arguments, char** arguments)
{
	...
	for(i = 0 ;i < NUM_OF_COMMANDS ;i++)
	{
		...
		{
			if(strcmp(arguments[0],commands[i].name)==0 && number_of_arguments-1 >= 1)
			{
				LIST_INIT(&foundCommands);
				return i;
			}
		}
		//case command found and number of argument right
		else if(strcmp(arguments[0],commands[i].name)==0 && commands[i].num_of_args ==number_of_arguments-1)
		{
			LIST_INIT(&foundCommands);
			return i;
		}

		//case found but number of arguments wrong
		else if(strcmp(arguments[0],commands[i].name)==0 && commands[i].num_of_args != number_of_arguments-1)
		{
